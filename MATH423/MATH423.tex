...
\end{itemize}
  ...
  Sampling distribution of $\hat{\beta}_0, \hat{\beta}_1$ and
    ...
    \begin{align*}
      ...
      \\ & \iff \frac{\hat{\beta}_1 - c}{ese(\hat{\beta}_1)} < -k \text{ or
           } \frac{\hat{\beta}_1 - c }{ese(\hat{\beta}_1)} > k
...
\end{align*}
 ...
 $\beta$ by using penalization, i.e.\
  ...
  \end{align*}
  \paragraph{Example}~
  \\$X_1$ --- Categorical: $A,B,C$
  \\$X_2$ --- Categorical: $Female, Male$
  \\$
  \begin{array}{c c c c}
    \hline X_1&X_1^{(1)}&X_1^{(2)}&X_{1}^{(3)}
    \\\hline A & 1 & 0 & 0
    \\ B & 0 & 1 & 0
    \\ C & 0 & 0 & 1
    \\\hline
  ...
  \begin{align*}
    E[Y \mid X] & = \underbrace{\beta_1 X_1^{(1)} + \beta_2 X_1^{(2)}
                  + \beta_3 X_1^{(3)}}_{X_1} + \underbrace{\beta_4
                  X_2^{(1)} + \beta_5X_2^{(2)}}_{X_2}
  \end{align*}
  \\ Model 2 (with main effect only, removing redundant dummy variables):
  \begin{align*}
    E[Y \mid X] & = \beta_0' + \beta_1'X_1^{(1)} + \beta_2'X_1^{(2)} + \beta_4'X_2^{(1)}
  \end{align*}
  $
  \begin{array}{c c c c c c}
    \hline X_1&X_2&X_1^{(1)}&X_1^{(2)}&X_{2}^{(1)} & \text{Model}
    \\\hline A & Female & 1 & 0 & 1 & \beta_0' + \beta_1' + \beta_4'
    \\ B & Female & 0 & 1 & 1 & \beta_0' + \beta_2' + \beta_4'
    \\ C & Female & 0 & 0 & 1 & \beta_0' + \beta_4'
    \\ \hline A & Male & 1 & 0 & 0 & \beta_0' + \beta_1'
    \\ B & Male & 0 & 1 & 0 & \beta_0' + \beta_2'
    \\ C & Male & 0 & 0 & 0 & \beta_0'
  \end{array}
  $
  \begin{align*}
    E[Y \mid X_1 = B, X_2 = Male] & = \beta_0' + \beta_2'
    & E[Y \mid X_1 = C, X_2 = Male] & = \beta_0'
  \end{align*}
  \begin{align*}
    \beta_2' & = \overbrace{E[Y \mid X_1 = B, X_2 = Male]}^{\circled{1}} = \overbrace{E[Y\mid X_1 = C,
                  X_2 = Male]}^{\circled{2}}
  \end{align*}
  $\beta_2'$ can be interpreted as the effect contrast between
  \circled{1} and \circled{2}.
  \\ Model 2 (with interaction terms):
  \begin{align*}
    E[Y \mid X] & = \beta_0' + \underbrace{\beta_1'X_1^{(1)} + \beta_2'X_{1}^{(2)}}_{X_1}
                  + \underbrace{\beta_4'X_2^{(1)}}_{X_2} +
                  \underbrace{\beta_{14}' X_1^{(1)}X_2^{(1)}}_{A} + \underbrace{\beta_{24}'X_1^{(2)}X_2^{(1)}}_{B}
  \end{align*}
  When there is $\beta_0$ total number of interactions between $X_1$
  ($M_1$ levels) and $X_2$ ($M_2$ levels) is $$(M_1 - 1) \times (M_2 -
  1) = (3-1) \times (2  - 1) =2$$
  $
  \begin{array}{c c c c c c}
    \hline X_1&X_2&X_1^{(1)}&X_1^{(2)}&X_{2}^{(1)} & E[Y \mid X_1, X_2]
    \\\hline A & Female & 1 & 0 & 1 & \beta_0' + \beta_1' + \beta_4' +
                                      \beta_{14}'
    \\ B & Female & 0 & 1 & 1 & \beta_0' + \beta_2' + \beta_4' + \beta_{24}'
    \\ C & Female & 0 & 0 & 1 & \beta_0' + \beta_4'
    \\ \hline A & Male & 1 & 0 & 0 & \beta_0' + \beta_1'
    \\ B & Male & 0 & 1 & 0 & \beta_0' + \beta_2'
    \\ C & Male & 0 & 0 & 0 & \beta_0'
  \end{array}
  $
  \\ Next two R computations posted on website will be \textbf{VERY}
