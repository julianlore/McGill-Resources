...
\end{align*}
 ...
 $\beta$ by using penalization, i.e.\
  ...
  \\ When $L=M \iff$ ``baseline level''
  \\ We may write $X_1$ --- categorical with $M$ levels
  \begin{align*}
    E[Y \mid X_1] & = \sum_{\ell = 1}^{M-1}B_{\ell}X_1^{(\ell)}
  \end{align*}
  ...
  \begin{align*}
    \beta_\ell^c & =
                   \begin{cases}
                     \beta_0 & \ell = 0
                     \\ \beta_\ell - \beta_0 & \ell = 1, \ldots, M-1
                   \end{cases}
    \\ E[Y \mid X_1] & = \sum_{\ell=1}^{M-1}\beta_\ell X_1^{(\ell)} =
                       \underbrace{\beta_0^c}_{= \beta_0} + \sum_{\ell = 1}^{M-1}\beta_\ell^c X_{1}^{\ell}
  \end{align*}
\end{document}