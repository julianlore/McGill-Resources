...
\end{align*}
\paragraph{Statistical properties of least-squares estimator}
...
\begin{align*}
  F_0 & = \dfrac{\overline{SS}_R(\beta_{(2)}\mid \beta_{(1)})/r}{\overline{SS}_{Res}/(n-p)}
        = \dfrac{\overline{SS}_R(\beta_{(2)}\mid \beta_{(1)})/r}{MS_{Res}}
  \\ \overline{SS}_R(\beta_{(2)} \mid \beta_{(1)}) & = \overline{SS}_R (\beta) - \overline{SS}_R (\beta_{(1)})
  \\ \overline{SS}_{R}(\beta) & = \hat{\beta}X^T Y & \overline{SS}_R (\beta_{(1)}) & = \hat{\beta}_{(1)}X_{(1)}^T Y
  \\ \hat{\beta} & = (X^TX)^{-1}X^TY & \hat{\beta}_{(1)} & = (X_{(1)}^TX_{(1)})^{-1}X_{(1)}^TY
  \\ MS_{Res} & = \dfrac{SS_{Res}}{n-p} = \dfrac{Y^TY - \hat{\beta}X^T Y}{n-p}
\end{align*}
$r$ --- degree of freedom for $\overline{SS}_R(\beta_{(2)} \mid
\beta_{(1)})$
\\ $p - (p-r) = r$
\end{document}