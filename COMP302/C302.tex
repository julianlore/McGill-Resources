...
\label{sec:org0f96aa7}
\begin{itemize}
\item Can do assignments in groups of 2
\end{itemize}

\subsection{Misc}
\label{sec:org57ced3a}
\begin{itemize}
...
\label{sec:orgd9c4a1e}
\subsection{Functions}
\label{sec:org5388844}
\begin{itemize}
...
\label{sec:org3be2889}
\subsubsection{Playing cards}
\label{sec:org5b3cb3a}
\begin{itemize}
...
\label{sec:org56efe68}
\begin{enumerate}
\item Step 1
\label{sec:org820bf70}
...
\end{itemize}
\item Theorem
\label{sec:org88072f2}
For all \(l, v, k\), lookup \(k\) (insert \((k,v) l\)) \(\implies *\) Some \(v\)
\item Proof
\label{sec:orgcd69835}
...
\label{sec:org7aacad0}
\subsection{Structural Induction}
\label{sec:org3824cda}
\begin{itemize}
...
\label{sec:orgd8a4238}
\subsection{Higher-order functions}
\label{sec:org956c5d8}
\begin{itemize}
...
\label{sec:orgd8cca68}
\subsection{Lambda-Calculus}
\label{sec:org8332ae7}
\begin{itemize}
...
\label{sec:org5b9eee2}
\subsection{Warm up}
\label{sec:org9ae78cb}
...
\label{sec:org0306cef}
\subsection{Demo}
\label{sec:orgc983b46}
\begin{itemize}
...
\label{sec:org9ea6857}
\subsection{Exceptions}
\label{sec:orgc282f05}
\begin{itemize}
...
\label{sec:orgd6c1ac4}
\subsection{Backtracking}
\label{sec:orgba6d22c}
\begin{itemize}
...
\label{sec:org5e36a0d}
\subsection{Modules}
\label{sec:orgb486428}
Primary Benefits:
\begin{itemize}
...
\label{sec:org15c82da}
\subsection{More on Modules}
\label{sec:orge9c4fac}
\begin{itemize}
...
\label{sec:org9805858}
\subsection{Continuations}
\label{sec:org63068a3}
...
\label{sec:orgaf780bf}
\subsection{Continuation recap}
\label{sec:org21cfda8}
\begin{itemize}
...
\label{sec:orga47726e}
\subsection{Regular expressions}
\label{sec:org57670bd}
...
\label{sec:orge38b04b}
\subsection{Lazy Programming}
\label{sec:org50761d0}
\subsubsection{Eager vs Lazy}
\label{sec:org57d6c18}
\begin{itemize}
...
\label{sec:org613a600}
\subsection{Demo}
\label{sec:orge9f298c}
\begin{verbatim}
...
\label{sec:org533c002}
\subsection{Designing our own language}
\label{sec:org0e9dbb7}
...
\label{sec:org5b89f4c}
We can run it!
\subsubsection{Static Type Checking}
\label{sec:org6382302}
\begin{itemize}
\item Types approximate runtime behavior
...
\end{verbatim}
\section{Lecture 28 \textit{<2017-11-16 Thu>}}
\label{sec:org808624c}
Extending our language with variables, let-expressions, and functions!
\begin{itemize}
\item Add the following expressions: \(x | let \ x = e_1 \ in \ e_2 \ end\)
\end{itemize}
Are the following well-formed?
\begin{itemize}
\item \texttt{let x = x in x + 1 end}
\begin{itemize}
\item x is not defined, not well-formed
\end{itemize}
\item \texttt{let x = y in x + 1 end}
\begin{itemize}
\item Same problem, y is not defined.
\item Is this equal to the line above? What kind of renaming do we want?
\end{itemize}
\item \texttt{let x = 1 in let y = x end in x + y end}
\begin{itemize}
\item Legal
\end{itemize}
\item \texttt{let x = 1 in let x = x + 2 end in x + x end}
\begin{itemize}
\item Legal, important to know where things are bound, the first x bounds the \(x\) in \(x+2\). The second let x statement bounds the \(x\) in \(x+x\)
\end{itemize}
\end{itemize}
\subsection{Free Variables}
\label{sec:orgadaefab}
Variable names should not matter

These are all the same: 
\begin{itemize}
\item \texttt{let x = 5 in (let y = x + 3 in y + y end) end}
\item \texttt{let x = 5 in (let x = x + 3 in x + x end) end}
\item \texttt{let v = 5 in (let w = v + 3 in w + w end) end}
\end{itemize}
...
\label{sec:orgc09c8f5}
Continuation from last class.
\subsection{Substitution}
\label{sec:org9fe1e06}
\begin{itemize}
...
\label{sec:orge29b38e}
\subsection{T-FN rule}
\label{sec:orgc488def}
...
\label{sec:org714fb51}
\subsection{Type Inference}
\label{sec:orgedc5af2}
...
\label{sec:orgf43096a}
\subsection{How to solve constraints?}
\label{sec:orgda67295}
...
\label{sec:orgf46ebd1}
\subsection{Subtyping}
\label{sec:orgee7900f}
\begin{itemize}
...
\end{itemize}
\section{Lecture 34 \textit{<2017-11-30 Thu>}}
\label{sec:org7f3b1dd}
\subsection{References}
\label{sec:org4f3ac41}
\begin{itemize}
\item Add new expression, add new expressions \(e_1 := e_2, !e, ref e, ()\) and add new types \(T ref, unit\)
\end{itemize}
...
Recall:
\begin{itemize}
\item What is the type of an assignment?
\end{itemize}
\begin{prooftree}
...
\begin{itemize}
\item Recall, subtyping:
\begin{itemize}
\item Base types are co-variant
\item Products are contra-variant in the input
\item Functions are co-variant in the output
\end{itemize}
\end{itemize}
...
!x * 3.14
\end{verbatim}
Is it safe to write \texttt{!y * 3.14} instead of \texttt{!x * 3.14}? i.e. Can we provide a \texttt{int ref} when a \texttt{float ref} is required. Yes, we can just downcast it. Seems to suggest:
...
y := 4
\end{verbatim}
Is it safe to write \texttt{x := 4} instead of \texttt{y :=4}? Can we provide a \texttt{float ref} when a \texttt{int ref} is required? Yes, just upcast it. Seems to suggest: 
...
\begin{itemize}
\item This is surprising, as in things like Java we do have subtyping with references?
\begin{itemize}
\item Subclassing is a form of subtyping
\item Nominal Subtyping: User declares what type (class) extends a subtype (subclass)
\item So in Java they have an unsound subtyping rule, but then during runtime they have an unsound check that checks if an assignment is valid
\begin{itemize}
\item Pay during runtime, have to keep types around and also check them during assignments
\end{itemize}
\item Typing and runtime diverge -> Runtime errors
\item In Java we upcast ourselves
\item Upcast an int array to a floating point array
\item Try to assign a floating point to this array will try to assign a floating point number into an int array
\end{itemize}
\end{itemize}
\subsection{Functional vs Object Oriented}
\label{sec:orgcd680df}
Functional
\begin{itemize}
\item Data and functions are decoupled (separate)
\end{itemize}
\begin{verbatim}
type people =
    | Staff of ...
...
\end{verbatim}
\begin{itemize}
\item Easy to add new functionality
\begin{itemize}
\item Just write a new function that matches based on type of person, for example, raising tuition of students only
\end{itemize}
\end{itemize}
Object Oriented
\begin{itemize}
\item Data and functionality are coupled
\begin{itemize}
\item Just make a subclass
\end{itemize}
\item Easy to add new data
\end{itemize}
...
\subsection{Effect of programming langauges on software quality}
\label{sec:orgff4a495}
A Large-Scale Study of Programming Languages and Code Quality in Github (paper made in 2017)

Summary of results:
\begin{itemize}
\item Functional languages better than procedural languages
\item Disallowing implicit type conversion (coercion) is better
\begin{itemize}
\item No subtyping, i.e. what OCaml does
\item Fewer errors
\end{itemize}
\item Static typing is better than dynamic
\item Managed memory usage is better than unmanaged
\begin{itemize}
\item C is unmanaged
\end{itemize}
\item The defect proneness of langauges is not associated with software domains (kind of problem we're solving)
\end{itemize}
\section{Lecture 35 \textit{<2017-12-01 Fri>}}
\label{sec:orgc614709}
\subsection{Four main goals of COMP302}
\label{sec:orge7ab6da}
1.| Provide a thorough introduction to fundamental concepts in programming languages
\begin{itemize}
\item Know how to use and write higher order functions
\item Used to model object enclosures
\begin{itemize}
\item Like counter
\item Tick
\item Memoization
...
\begin{itemize}
\item Defer control
\item Backtrack through a tree
\item Coins and change problem
\end{itemize}
\item Continuations
\begin{itemize}
\item Alternative to exceptions
\item Used for many things
\item Form of higher order functions
\item Used to get rid of exceptions
\item Use to turn function into tail recursive
\item Success continuations to accumulate a certain result
\item Failure continuations to go back when we need it to stop
\end{itemize}
\item State
\begin{itemize}
\item 2 kinds of variables
\item Bound variables, introduced in a let expression, function, pattern
\item Global variable, introduced when introducing state with ref
\end{itemize}
\item Lazy programming
\begin{itemize}
\item Higher order functions used to delay evaluation like for the tail of a stream
\end{itemize}
\item Partial evaluation
\begin{itemize}
\item Played a roll in object enclosures to trap state
\end{itemize}
\item Modules
\begin{itemize}
\item Polymorphism
\end{itemize}
\end{itemize}
Most languages support some kind of higher order functions, even Java, but not as powerful as OCaml
2.| Show different ways to reason about programs
\begin{itemize}
\item Type checking
\begin{itemize}
\item Should improve your understanding of what went wrong in your program
\item Thinking about types is important even if you aren't using a static language
\item Types help you improve features and understanding of programs
\end{itemize}
\item Induction
\begin{itemize}
\item Everyone should be able to do an induction proof, guaranteed to have one on the final
\end{itemize}
\item Operational semantics
\begin{itemize}
\item How does this evaluate?
\item How do local variables and references come into play?
\item What is the effect of a program?
\end{itemize}
\end{itemize}
3.| Introduce fundamental principles in programming language design
\begin{itemize}
\item How should we go about designing languages?
\item Grammar and parsing required
\begin{itemize}
\item Syntax of the language, allows you to understand it
\item Syntax may differ from functional languages, but they are more or less the same
\item Differ more in type checking and operational semantics
\item Haskell has very similar syntax but different semantics, lazy language in how it executes
\end{itemize}
\item Operational semantics
\item Interpreters
\item Type checking
...
\item Subtyping
\begin{itemize}
\item How do people implement subtyping into their language?
\begin{itemize}
...
\begin{itemize}
\item Helps to be exposed to different ways of thinking
\item Used OCaml in this course to do this
\item Could have been another language
\item Yes, we used a language to go through these concepts, but these concepts are very general
\end{itemize}

\noindent\rule{\textwidth}{0.5pt}
\subsection{General Principles}
\label{sec:org5593f1a}
\begin{enumerate}
\item If you learn a new language, learn one that changes the way you think about programming. Or else it's just learning syntax.
\item Simple and elegant solutions are much more effective, but complex solutions are easier to find and require less time, but in the end it is worth it
\end{enumerate}
\subsection{Exam}
\label{sec:org09fb0e4}
\begin{itemize}
\item 8 questions
\item First question is how expressions are typed
\begin{itemize}
\item A bit like on the midterm
\item \texttt{fun x y -> if x * (-1) < 0 then (y,x) else (x,y)}
\begin{itemize}
\item What is the most general type of this expression?
\item \texttt{int -> int -> int * int}
\end{itemize}
\end{itemize}
\item Understand the difference between values and effects
\item Coin problem revisited
\begin{itemize}
\item Implemented using continuations this time
\end{itemize}
\item Lists and trees are bread and butter
\item Revisited count and made it tail recursive
\end{itemize}
\end{document}