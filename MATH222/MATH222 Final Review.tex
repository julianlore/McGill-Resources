...
\end{align*}
\subsection{Spherical Coordinates} Represent a point $P$ by $(\rho,\theta,\phi)$. $\rho$ is the distance from $P$ to origin ($\rho \geq 0$). $\phi$ is the angle between positive $z$ axis and the line from origin to $P$, $0\leq \phi \leq \pi$
