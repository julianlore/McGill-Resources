...
suc \ n \to (le_s\ n\ n) (f\ n)$
	...
	\begin{scprooftree}{0.7}
		...
		\AXC{$\mathcal{D}_1$}
		\noLine
		\UIC{$\mathcal{Y}, a : nat \vdash \neg (z = z) \supset \exists y:
				nat. \ suc\ y = z$}

		\AXC{$\mathcal{D}_2$}
		\noLine
		\UIC{$\mathcal{Y}, a:nat, n : nat, ih = \ldots  \vdash \neg(suc\ n = z)
...
\begin{prooftree}
	\AXC{}
	\RL{$n$-times}
	\UIC{$\Delta; x_1 : B_1, \ldots, x_n : B_n \vdash A$ true}
	\RL{$\supset$ I}
...
\begin{prooftree}
	...
	\RL{$\supset$ L}
	\BIC{$\Delta; \Gamma, x : A \supset B \implies C$}
\end{prooftree}
\begin{prooftree}
	\AXC{}
	\RL{initial}
...
\begin{prooftree}
	...
	\BIC{$\Delta, u:\underbrace{[\psi]A}_{\psi \supset A}; \Gamma \implies C$}
      ...
      \\ Intuitively:
      \begin{prooftree}
        \AXC{$\Theta_n; \ldots ; \Theta_{i-1}; \Gamma_i \vdash \rho_i : \Theta_i$}
